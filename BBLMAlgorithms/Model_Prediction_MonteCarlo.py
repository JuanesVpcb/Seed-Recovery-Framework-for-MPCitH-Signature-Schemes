    ...
    try:
        for it in range(N):
            try:
                ...
                L = generate_candidates(P, W, w, eta, mu, scale)
...
def estimate_Psuccess_from_montecarlo(W, mu, w, eta, alpha, beta, Bmin, B2, base_path):
    """
    Devuelve una aproximación de Psuccess usando archivos Monte Carlo en formato CSV.
    Si el archivo no existe o está vacío (sin datos válidos), devuelve 0.0.

    Parámetros:
        W, mu, w, eta, alpha, beta, Bmin, B2 : parámetros de la configuración
        base_path (str): carpeta donde están los archivos CSV del Monte Carlo
    """
    filename = f"W{W}_mu{mu}_w{w}_eta{eta}_a{int(alpha*1000)}_b{int(beta*100)}_binned_pow2.csv"
    path = os.path.join(base_path, filename)

    # --- Verificación de existencia ---
    if not os.path.exists(path):
        print(f"⚠️  Archivo MonteCarlo no encontrado: {path}")
        return 0.0

    try:
        # Leer el archivo CSV (una sola fila)
        df = pd.read_csv(path)
        print(f"📊 Procesando Monte Carlo: {path}")
        print(df)
        if df.empty:
        ...
        # Verificar si hubo simulaciones válidas
        if ("N_valid" in row and row["N_valid"] == 0) or ("excluded_rate" in row and row["excluded_rate"] >= 1.0):
            print(f"⚠️  Monte Carlo inválido o sin datos útiles (excluded_rate=1.0 o N_valid=0): {path}")
            return 0.0

        # Parsear las listas (edges y CDF)
        edges = ast.literal_eval(str(row["edges_last"]))
        cdf   = ast.literal_eval(str(row["C_pref_rate"]))
        print(f"    edges: {edges}")
        print(f"    CDF: {cdf}")

        # Validar contenido
        if not edges or not cdf or len(edges) != len(cdf)+1:
            print(f"⚠️  CDF o edges vacíos o inconsistentes en: {path}")
            return 0.0

        # Buscar índices de Bmin y B2
        idx_B2 = bisect.bisect_left(edges, B2)
        idx_B2 = min(idx_B2, len(cdf) - 1)

        idx_Bmin = bisect.bisect_left(edges, Bmin)
        F_B2 = cdf[idx_B2]
        F_B1 = cdf[idx_Bmin - 1] if idx_Bmin > 0 else 0.0

        # Calcular probabilidad acumulada
        P_success = max(F_B2 - F_B1, 0.0)

        # Ajustar por tasa de exclusión si existe
        excluded_rate = 0#  float(row.get("excluded_rate", 0))
        P_success *= (1 - excluded_rate)

        return float(P_success)

    except Exception as e:
        print(f"⚠️  Error procesando Monte Carlo {path}: {e}")
        return 0.0

def seedRecovery_MC_until24(
    ...
    *,
    # Presupuestos
    base_path_montecarlo,    # <--- NUEVO PARÁMETRO
    ...
    for (W, mu, w, eta) in G:
        ...
        L = generate_candidates(P_post, W, w, eta, mu, scale)
        ...
        x = np.array([[float(W), float(w), float(mu), float(eta),
                     ...
                     dtype="float32")
        P_hat = estimate_Psuccess_from_montecarlo(
            W, mu, w, eta, alpha, beta, Bmin, B2_star,
            base_path=base_path_montecarlo  # <-- nuevo parámetro
        )
...
W_vals     = [128,192,256]                          # longitud total de la semilla
...
resultados_dir = "resultados_MC_v6"
errores_dir    = "errores_MC_v6"
resultados_basepath_montecarlo = "resultados_v6"  # ruta base para archivos MC previos
...
# ---------------------------------------------------------------------
# ✅ Función individual que ejecuta un job completo (una configuración)
# ---------------------------------------------------------------------
def run_single_job(params):
    """
    Ejecuta una combinación de parámetros y guarda el resultado JSON.
    """
    (W, mu, w, eta, alpha, beta,
     Btime, Bmemory, Cbase, Cblock, Coracle,
     resultados_dir, errores_dir, resultados_basepath_montecarlo,
     scale, overwrite) = params

    import time
    gid = (f"W{W}_mu{mu}_w{w}_eta{eta}_a{int(alpha*1000)}_b{int(beta*100)}"
           f"_Bt{int(np.log2(Btime))}_Bm{int(np.log2(Bmemory))}"
           f"_Cb{int(np.log2(Cbase))}_Cbl{int(np.log2(Cblock))}_Co{int(np.log2(Coracle))}")

    path_json = os.path.join(resultados_dir, f"{gid}.json")

    # Saltar si ya existe
    if os.path.exists(path_json) and not overwrite:
        return {"status": "SKIPPED", "group_id": gid, "path": path_json}

    try:
        # Generar seed y posteriors
        random.seed(42)
        s_star = bitarray([random.getrandbits(1) for _ in range(W)])
        s_tilde = decay_seed(s_star, alpha, beta)
        ...
        data = {
            "status": "EMPTY", "group_id": gid, "error": str(e),
            "W": W, "mu": mu, "w": w, "eta": eta,
            "alpha": alpha, "beta": beta,
    ...
    try:
        out = seedRecovery_MC_until24(
            P_post=P_post,
            G=[(W, mu, w, eta)],
            alpha=alpha, beta=beta,
            base_path_montecarlo=resultados_basepath_montecarlo,
            Btime=Btime, Bmemory=Bmemory,
    ...
    # Construir salida
    if out is None:
        data = {"status": "EMPTY", "group_id": gid}
    else:
        data = {
            "status": "OK",
            "group_id": gid,
    ...
    del s_star, s_tilde, P_post, out
    gc.collect()
    return data

# ---------------------------------------------------------------------
# 🚀 Bloque principal (solo se ejecuta una vez en Windows)
# ---------------------------------------------------------------------
if __name__ == "__main__":
    # === Parámetros de la grid ===
    W_vals = [128, 192, 256]
    alpha_list = [0.001]
    beta_list  = [0.03, 0.05, 0.10]
    Btime_list   = [2**30]
    Bmemory_list = [2**30]
    Cbase_list   = [2**5]
    Cblock_list  = [2**4]
    ...
    # 🧩 Generar dinámicamente los parámetros válidos
    grid = list(generar_parametros_validos(W_vals))
    print(f"🧮 {len(grid)} configuraciones válidas generadas dinámicamente")

    # === Paths ===
    resultados_dir = "resultados_test"
    errores_dir = "errores_test"
    resultados_basepath_montecarlo = "resultados_v6"
    Path(resultados_dir).mkdir(exist_ok=True)
    Path(errores_dir).mkdir(exist_ok=True)

    scale = 10_000
    overwrite = False

    # === Generar combinaciones completas ===
    jobs = [
        (*cfg, alpha, beta, Btime, Bmemory, Cbase, Cblock, Coracle,
         resultados_dir, errores_dir, resultados_basepath_montecarlo,
         scale, overwrite)
        for (cfg) in grid
    ...
    print(f"⚙️ Ejecutando en paralelo con {n_workers} procesos...")
    t0 = time.time()
    rows = []
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures), 1):
            try:
                ...
                rows.append(result)
                print(f"[{i}/{len(jobs)}] {result['group_id']} → {result['status']}")
            except Exception as e:
                print(f"⚠️ Error en tarea {i}: {e}")

    # === Guardar resumen ===
    df = pd.DataFrame(rows)
    summary_path = os.path.join(resultados_dir, "summary.csv")
    df.to_csv(summary_path, index=False)
    print(f"\n✅ Paralelización completada: {len(rows)} combinaciones en {time.time()-t0:.1f}s")
